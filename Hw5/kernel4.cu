...
__global__ void mandelKernel(int *d_out, float lowerX, float lowerY,
                             float stepX, float stepY, int resX,
                             int maxIterations) {
...
void hostFE(float upperX, float upperY, float lowerX, float lowerY, int *img,
  ...
  mandelKernel<<<numBlocks, threadsPerBlock>>>(d_out, lowerX, lowerY, stepX,
                                               stepY, resX, maxIterations);
