...
__global__ void mandelKernel(int *d_data, int width, float stepX, float stepY,
                             float lowerX, float lowerY, int count) {
...
void hostFE(float upperX, float upperY, float lowerX, float lowerY, int *img,
  ...
  mandelKernel<<<numBlocks, threadsPerBlock>>>(d_data, resX, stepX, stepY,
                                               lowerX, lowerY, maxIterations);
