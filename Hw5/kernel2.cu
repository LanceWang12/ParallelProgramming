...
__global__ void mandelKernel(int *d_out, size_t pitch, float lowerX,
                             float lowerY, float stepX, float stepY, int resX,
                             int maxIterations) {
...
void hostFE(float upperX, float upperY, float lowerX, float lowerY, int *img,
  ...
  cudaMallocPitch(&d_out, &pitch, resX * sizeof(int), resY);

  // CUDA function
  dim3 threadsPerBlock(BLOCK_SIZE, BLOCK_SIZE);
  dim3 numBlocks(resX / threadsPerBlock.x, resY / threadsPerBlock.y);
  mandelKernel<<<numBlocks, threadsPerBlock>>>(
      d_out, pitch, lowerX, lowerY, stepX, stepY, resX, maxIterations);

  cudaMemcpy2D(h_out, resX * sizeof(int), d_out, pitch, resX * sizeof(int),
