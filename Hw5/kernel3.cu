...
__global__ void mandelKernel(int *d_data, float stepX, float stepY,
                             float lowerX, float lowerY, int count, int pitch,
                             int scale) {
...
void hostFE(float upperX, float upperY, float lowerX, float lowerY, int *img,
  ...
  dim3 numBlocks(resX / threadsPerBlock.x / scale,
                 resY / threadsPerBlock.y / scale);
  mandelKernel<<<numBlocks, threadsPerBlock>>>(
      d_data, stepX, stepY, lowerX, lowerY, maxIterations, pitch, scale);

  cudaMemcpy2D(data, resX * sizeof(int), d_data, pitch, resX * sizeof(int),
               resY, cudaMemcpyDeviceToHost);
